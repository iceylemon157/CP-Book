...
> 2. $$p_{i+1}\neq i$$：將 (1) 的$$j$$代入$$p_{i+1}$$，(2) 的$$j$$代入$$p_i$$可以得到下面兩個式子：\
>    \
>    $$dp_{p_i}+w(i,p_i)\leq dp_{p_{i+1}}+w(i,p_{i+1})$$\
>    $$dp_{p_{i+1}}+w(i+1,p_{i+1})\leq dp_{p_i}+w(i+1,p_i)$$\
>    \
>    把這兩個式子相加可以得到：\
>    \
>    $$w(i,p_i)+w(i+1,p_{i+1})\leq w(i,p_{i+1})+w(i+1,p_i)$$\
>    \
>    根據四邊形不等式可以利用反證證出$$p_i\leq p_{i+1}$$，證畢。
...
單調隊列優化跟斜率優化都對$$w(i,j)$$有比較特別的要求。具體來說，要可以把$$i,j$$分離到一定程度。那麼如果$$w$$只滿足四邊形不等式呢？舉例來說：$$w(i,j)=-\sqrt{i+j}$$(關於為什麼他滿足四邊形不等式就繼續往下看ㄅ)。而即使$$dp$$滿足最優決策點遞增，也沒辦法用類似單調隊列的方法快速求出$$dp(i)$$。(考慮決策點永遠都是$$j=1$$的情況，如此一來還是需要$$O(N^2)$$的時間來處理。)
...
這裡要先提一點，如果$$dp(i,j)$$滿足決策點單調的話，$$w(k,j)$$不一定要滿足四邊形優化(因為決策點單調不能推回去四邊形不等式成立)。**也就是說分治優化不一定需要滿足四邊形不等式**(但作法是一樣的)。我們這邊說明滿足四邊形不等式的分治優化：
...
$$dp(i,j)=\min\limits_{k<j}\{dp(i-1,k)+w(k+1,j)\}$$(這個就是上面分治優化的轉移式)
...
上面這個作法其實是$$O(N^2)$$的，這邊給出本章節的唯二個證明(只證明第一式)：
...
至此我們也解決了 2D/1D 的問題。而我們也給出了在滿足區間單調性的條件下，式(2) 甚至可以達到$$O(N^2)$$的複雜度。而值得注意的是，如果$$w$$沒辦法快速計算的話，這個方法的複雜度一樣會退化。所以還是需要考慮使用分治優化。在實際運用上需要多加注意。

### 題目

1. [Atcoder DP Contest N](https://atcoder.jp/contests/dp/tasks/dp\_n)
...
1. $$\forall i<j$$，$$w(i,j)+w(i+1,j+1)\leq w(i+1,j)+w(i,j+1)$$跟$$w$$滿足四邊形不等式是等價條件。
2. 在 2D/1D DP 的部分我們寫了$$p(i,j-1)\leq p(i,j)\leq p(i+1,j)$$，整理一下之後可以變成$$p(i-1,j)\leq p(i,j)\leq p(i,j+1)$$。寫成這個形式可以讓你枚舉的時候方便一點。
...
3. 令$$h(x)$$是一個**單調遞增的凸函數**(convex)。若$$w(i,j)$$滿足**四邊形不等式並且具有區間單調性**，那麼$$h(w(i,j))$$也滿足四邊形不等式以及區間單調性。
