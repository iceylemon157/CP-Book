...
所以接下來我們要介紹兩個**如果**$$w$$**滿足四邊形不等式**的通用作法：
...
$$dp(j)=\min\limits_{k<j}\{g(k)+w(k,j)\}$$
...
這裡要補充一點，我不太確定如果$$dp(j)$$滿足決策點單調的話，$$g(k)+w(k,j)$$會不會一定滿足四邊形優化，我不會證明QQ。所以這個結論還是記起來吧\(雖然分治優化不太常用\)。我們下面繼續用四邊形優化來說明這個轉移式：
