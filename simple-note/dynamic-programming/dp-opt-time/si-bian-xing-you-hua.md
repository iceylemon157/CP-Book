...
單調隊列優化跟斜率優化都對$$w(i,j)$$有比較特別的要求。具體來說，要可以把$$i,j$$分離到一定程度。那麼如果$$w$$只滿足四邊形不等式呢？舉例來說：$$w(i,j)=-\sqrt{i+j}$$\(關於為什麼他滿足四邊形不等式就繼續往下看ㄅ\)。而即使$$dp$$滿足最優決策點遞增，也沒辦法保證可以在高速的時間內求出$$dp(i)$$。\(考慮決策點永遠都是$$j=1$$的情況，如此一來還是需要$$O(N^2)$$的時間來處理。\)
