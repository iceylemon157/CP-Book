...
這裡要補充一點，我不太確定如果$$dp(j)$$滿足決策點單調的話，$$g(k)+C(k,j)$$會不會一定滿足四邊形優化，我不會證明QQ。所以這個結論還是記起來吧\(雖然分治優化不太常用\)。我們下面繼續用四邊形優化來說明這個轉移式：

因為我比較喜歡$$i$$，我們把上面那個式子換一些符號重新整理一下：

$$dp(i)=\min\limits_{j<i}\{g(j)+w(i,j)\}$$

而事實上我們有一件事情：如果$$g(j)+w(i,j)$$滿足四邊形不等式，那麼$$w(i,j)$$滿足四邊形不等式。\(直接帶進四邊形不等式就顯然了\)。而只要滿足上面這個式子，我們可以考慮使用以下分治方法解決：
