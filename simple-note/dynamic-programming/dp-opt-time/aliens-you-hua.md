...
那麼我們就把原圖的所有白邊邊權加上$$k$$再跑一次 Kruskal 就可以得到$$g(x)$$了！

雖然我們看起來已經做完了，但是事實上我們漏掉了一個重要的Case：

聰明如你一定可以想到一個簡單的做法：**小數二分**。但是小數二分的效率並不高，在某些題目可能會 TLE。事實上很多時候可以透過題目性質來避免掉小數二分。例如這題，

至此再結合上面所述的二分搜就可以以$$O(NlogNlogC)$$的複雜度解掉這題了。
